...
/obj/screen/human/toggle/Click()
	...
	var/obj/item/holding = H.get_active_hand()
	if(holding)
		if(holding.slot_flags & SLOT_ICLOTHING)
			usr << "<span class='notice'>You start changing your uniform.</span>"
			if(do_after(H, 20))
				if(H.change_uniform(holding))
					usr << "<span class='notice'>You change your uniform.</span>"
					return
...
/datum/hud/proc/human_hud(ui_style = 'icons/mob/screen_midnight.dmi')
	...
	inv_box = new /obj/screen/inventory()
	inv_box.name = "right arm"
	inv_box.icon = ui_style
	inv_box.icon_state = "right arm"
	inv_box.screen_loc = ui_r_arm
	inv_box.slot_id = slot_r_arm
	inv_box.layer = 19
	other += inv_box

	inv_box = new /obj/screen/inventory()
	inv_box.name = "left arm"
	inv_box.icon = ui_style
	inv_box.icon_state = "left arm"
	inv_box.screen_loc = ui_l_arm
	inv_box.slot_id = slot_l_arm
	inv_box.layer = 19
	other += inv_box
