...
//Quickly change one's uniform without dropping anything, similar to goon.
/mob/living/carbon/human/proc/change_uniform(obj/item/I)
	if( !(I.slot_flags & SLOT_ICLOTHING) )
		return 0
	if(w_uniform)
		put_in_active_hand(w_uniform)
	equip_to_slot(I, slot_w_uniform)
	update_inv_l_hand()
	update_inv_r_hand()
	return 1